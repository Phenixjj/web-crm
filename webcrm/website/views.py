...
# This file contains the views for the website application.
...
def register_user(request):
    ...
    It sends an activation link to the user's email.
    """
...
def activate(request, uidb64, token):
    """
    This view handles the activation of a user account.
    It checks the token and activates the user account if the token is valid.
    """
    User = get_user_model()
    try:
...
def logout_user(request):
    """
    This view handles the logout of a user.
    """
    logout(request)
...
def dashboard(request):
    ...
    It fetches the last order for the logged-in user and the sales data for all products.
    """
    ...
    products_types_order = (
        Order.objects.all()
        .values("product__name")
        .annotate(total_order=Count("product"))
    )
